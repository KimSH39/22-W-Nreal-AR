...
namespace NRKernal.Experimental
{
    ...
    public class OverlayBase : MonoBehaviour, IComparable<OverlayBase>
    {
        ...
        public override string ToString()
        {
            ...
            {
                return string.Format("LayerId:{0}, go:{1}, depth:{2}, viewIndex:{3}, BufferSpec:{4}\nviewPort0:{5}", m_LayerId, gameObject.name, compositionDepth, ViewPorts[0].index, m_BufferSpec.ToString(),
                    ViewPorts[0].ToString());
            }
            else if (ViewPorts.Length == 2)
            {
                return string.Format("LayerId:{0}, go:{1}, depth:{2}, viewIndex:{3}_{4}, BufferSpec:{5}\nviewPort0:{6}\nviewPort1:{7}", m_LayerId, gameObject.name, compositionDepth, 
                    ViewPorts[0].index, ViewPorts[1].index, m_BufferSpec.ToString(),
                    ViewPorts[0].ToString(),
                    ViewPorts[1].ToString());
            }
            return string.Empty;
        }
    }
