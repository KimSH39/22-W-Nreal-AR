...
namespace NRKernal.Experimental
{
    ...
    internal partial class NativeSwapchain
    {
        ...
        internal void AcquireCameraFrame(UInt64 swapchainHandle, ref UInt64 frameHandler, int[] layerids, ref IntPtr[] layerbuffers)
        {
            ...
            // NRDebugger.Info("[NativeSwapchain] AcquireCameraFrame: frameCnt={4}, index1:{0}, index2:{1}, RenderHandler:{2} frameHandler:{3}", 
            //     layerids[0], layerids[1], RenderHandler, frameHandler, Time.frameCount);
        ...
        internal void SubmitFrame(UInt64 frameHandle, UInt64 viewPortList)
        {
            if (frameHandle == 0)
            {
                return;
